  ...
  users.forEach((user, idx) => {
    tableBody.innerHTML += `
        <tr>
            ...
            <td>${user.email}</td>
            <td>${user.age}</td>
            <td>    <button class ="btn btn-warning btn-sm p-2 m-2" onclick="editData(${idx})">Edit </button>
                    <button class ="btn btn-danger btn-sm p-2 m-2" onclick="deleteData(${idx})">Delet </button>
            </td>
            </tr>
        `;
  });
